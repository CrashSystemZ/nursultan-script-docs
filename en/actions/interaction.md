...
| `combat.explosionExposure(target, source, cover)` | `float` | same, counting boxes that are not in the world yet (API 6) (main thread only) |
...
| `combat.explosionDamage(target, source, power, cover)` | `float` | same, counting boxes that are not in the world yet (API 6) (main thread only) (throws `ScriptException` when `power` is 0 or less) |
...
| `interaction.useBlock(x, y, z, side, hand, hit)` | `void` | interacts at `hit` in world coordinates, null means the face centre (API 6) (main thread only) |
| `interaction.placeBlock(x, y, z, side, hand, hit)` | `void` | the same with a hit point, delegates to `useBlock` (API 6) (main thread only) |
...
| `interaction.pendingAck()` | `boolean` | your block use, item use or block break awaits the server's acknowledgement (API 7) (main thread only) |
