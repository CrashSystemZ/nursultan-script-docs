...
`ApiVersion.CURRENT` is 8. `requireApi(n)` fails the script at load when the running client is older; it cannot rescue a compile error, because a name that does not exist on the older SDK never compiles in the first place.
...
|---|---|---|
| `ApiVersion.CURRENT` | `int` | script API version of this client, currently 8 |
...
`ApiVersion` has a private constructor: no instance, only the two static members, and the client appends `this client provides v8` to every `Unresolved reference` compile error.
