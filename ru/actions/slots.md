...
| `slots.hold(hotbarSlot)` | `HeldSlot` | переиспользует живую ручку, если слот уже выбран, иначе выбирает его (API 6) (только главный поток) (бросает `ScriptException`, если слот не из хотбара) |
| `slots.selected()` | `Slot` | слот хотбара, выбранный прямо сейчас (API 6) (только главный поток) |
| `slots.using(slot) { }` | `T` | выбирает слот, выполняет блок, вызывает `restoreWhenSafe()` в `finally` (только главный поток) |

Все бросают `ScriptStateException`, если мира нет и после выгрузки скрипта. Все живые ручки скрипта возвращаются молча, когда скрипт выгружается.
Отложенный возврат клиент делает на своём тике, поэтому ручка не доказывает, что в руке: `hold` и `holding()` сверяются с реально выбранным слотом.
...
| `holding()` | `boolean` | этот слот всё ещё выбран и ручка не израсходована (API 6) |
