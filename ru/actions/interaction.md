...
| `combat.explosionExposure(target, source, cover)` | `float` | то же с учётом коробок, которых в мире ещё нет (API 6) (только главный поток) |
...
| `combat.explosionDamage(target, source, power, cover)` | `float` | то же с учётом коробок, которых в мире ещё нет (API 6) (только главный поток) (бросает `ScriptException`, если `power` не больше 0) |
...
| `interaction.useBlock(x, y, z, side, hand, hit)` | `void` | взаимодействует в точке `hit` в координатах мира, null — центр грани (API 6) (только главный поток) |
| `interaction.placeBlock(x, y, z, side, hand, hit)` | `void` | то же самое с точкой попадания, делегирует `useBlock` (API 6) (только главный поток) |
...
| `interaction.pendingAck()` | `boolean` | твоё использование блока или предмета либо ломание ждёт ответа сервера (API 7) (только главный поток) |
