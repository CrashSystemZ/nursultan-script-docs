...
**`slots.using(slot) { }` одалживает хотбар и возвращает его.** Блоки ищутся вопросом `placeable()` к каждому предмету хотбара, а подмена живёт ровно столько, сколько постановка и взмах, — тик никогда не заканчивается с чужим предметом в руке.
