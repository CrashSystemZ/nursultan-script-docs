...
Любое изменение требует открытого screen handler, а при открытом не-игровом экране слоты `ARMOR` и `OFFHAND` бросают `ScriptStateException`; батч ничего не блокирует, его очередь досылается в следующие тики, а батч без кликов и задержек отбрасывается вместе со своим `onFinish`.
