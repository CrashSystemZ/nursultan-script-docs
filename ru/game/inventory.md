...
`onFinish { ... }` выполнит твой код, когда батч закончился — все шаги ушли и инвентарь закрыт. Удобно продолжать цепочку, не карауля `busy()` каждый тик:

```kotlin
inventory.batch {
    it.swap(slot, hotbar)
    it.onFinish {
        // батч отработал, инвентарь закрыт
    }
}
```

Пустой батч отбрасывается целиком, вместе со своим `onFinish`.
