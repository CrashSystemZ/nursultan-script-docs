...
| `block.collisionBoxes()` | `List<Box>` | части формы коллизии в мировых координатах, пусто если нет (API 3) |
| `block.outlineBoxes()` | `List<Box>` | части формы обводки в мировых координатах, пусто если нет (API 3) |
...
Это те же формы, о которые режется `raycast.blocks(...)`; `block.box()` так и остаётся кубом 1×1×1.
