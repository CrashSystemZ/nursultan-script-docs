...
| Метод | Тип | Описание |
|---|---|---|
| `r.pushScissor(x, y, width, height)` | `void` | обрезает следующие команды прямоугольником, пересечённым с внешним |
| `r.popScissor()` | `void` | возвращает предыдущую обрезку, без эффекта при пустом стеке |

Обрезка действует с места команды в очереди, так что режется только выданное между push и pop; незакрытый push закрывается на выходе из обработчика и до других скриптов не доходит. Размытие читает фреймбуфер без обрезки, режется только его результат.
