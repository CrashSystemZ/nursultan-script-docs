...
Каждое отсечение, оставленное обработчиком рендера открытым, снимается при его возврате, а размытие читает кадр без отсечения — режется только его результат.
