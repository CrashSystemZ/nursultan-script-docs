...
Обрезка действует с места команды в очереди, так что режется только выданное между push и pop; незакрытый push заканчивается вместе с кадром. Размытие читает фреймбуфер без обрезки, режется только его результат.
