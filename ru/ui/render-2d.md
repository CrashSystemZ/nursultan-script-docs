...
Событие `Render2DEvent` вызывается каждый кадр на слое оверлея клиента — поверх игрового HUD, под элементами HUD клиента. Оно несёт `render()`, размер экрана `width()` × `height()` и `tickDelta()`.
...
Отсюда одно следствие: не зашивай размеры цифрами, если хочешь одинаковый вид на разных мониторах. Ставь всё от `width()` и `height()`, а размер шрифта считай от них же.
...
val logo = image("logo.png")                               // файл из папки скриптов
