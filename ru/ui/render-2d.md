...
Цвета, которыми рисуются меню и HUD клиента. `theme` — корень DSL, `client.theme()` — тот же объект; каждое значение — ARGB на момент вызова, поэтому производные от акцента меняются сразу, как только пользователь выбрал другой акцент.
