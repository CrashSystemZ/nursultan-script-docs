...
| Метод | Тип | Описание |
|---|---|---|
| `settings()` | `List<Setting>` | все настройки, созданные скриптом, в порядке создания, копия |

`SettingHost` — интерфейс, на котором объявлены создатели выше; вложенные настройки тоже попадают в список.
